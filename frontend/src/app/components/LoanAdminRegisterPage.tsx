...
import {
  ...
  IdCard, Briefcase, MapPin, Lock, Eye, EyeOff,
  CheckCircle2, ChevronDown, Bell, Settings, LogOut,
  Edit3, Landmark, FileSearch, AlertTriangle,
} from "lucide-react";
...
// ── Input class ───────────────────────────────────────────────────────────────
const inputCls =
  "w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl bg-slate-50/70 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-400 transition-all text-sm font-medium outline-none placeholder:text-slate-300";
...
function ProfileDropdown({
  ...
  const menuItems = [
    ...
    { icon: Landmark, label: "Loan Settings", action: () => {} },
    { icon: Bell, label: "Notifications", action: () => {} },
    { icon: Settings, label: "Security Settings", action: () => {} },
  ];
      ...
      >
        ...
        <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 p-5 text-white">
          ...
          <div className="flex items-center gap-3 mb-3">
            <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-lg font-black border-2 border-white/40 shrink-0">
              {initials}
            </div>
        ...
        <div className="p-2">
          {menuItems.map((item) => (
            <button
              key={item.label}
              onClick={item.action}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-blue-700 transition-colors text-left"
            >
...
// ── Main Component ────────────────────────────────────────────────────────────
export function LoanAdminRegisterPage() {
  const navigate = useNavigate();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.department) { toast.error("Please select a Department / Unit."); return; }
    ...
    try {
      ...
      localStorage.setItem("loanAdminProfile", JSON.stringify(profile));
      ...
      setIsLoading(false);
      setRegistered(true);
      toast.success("Loan Admin registered successfully!");
    } catch (error) {
      console.error("Loan admin register error:", error);
  ...
  if (registered && savedProfile) {
    const initials = getInitials(savedProfile.fullName);
    return (
        ...
        <div className="absolute inset-0 z-10 bg-black/60 backdrop-blur-[12px] flex items-center justify-center p-4">
          ...
          >
            <Card className="border-0 shadow-2xl rounded-2xl bg-white overflow-hidden">
              ...
              <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 px-8 py-8 text-white text-center">
                <motion.div
                ...
                <p className="text-blue-100 text-sm mt-1">{savedProfile.email}</p>
              ...
              <CardContent className="p-6">
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {[
                    ...
                    { label: "Employee ID", value: savedProfile.employeeId },
                    { label: "Organization", value: savedProfile.orgName },
                ...
                <div className="flex gap-3">
                  <Button
                    onClick={() => navigate("/admin/loan/dashboard")}
                    className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold rounded-xl h-11 shadow-lg shadow-blue-200"
                  >
                    Go to Dashboard
                  </Button>
                  <div className="relative">
  ...
  // ── Registration Form ──────────────────────────────────────────────────────
  const avatarInitials = getInitials(form.fullName);
  ...
  return (
    <div className="min-h-screen relative w-full overflow-hidden">
      ...
      <motion.div
        ...
        style={{ filter: "blur(6px) brightness(0.82)" }}
      >
        <HomePage />
      </motion.div>
      ...
      <div className="absolute inset-0 z-20 overflow-y-auto flex items-start justify-center p-4 py-10">
        ...
        >
          <Card className="border border-white/20 shadow-[0_32px_80px_rgba(0,0,0,0.55)] rounded-2xl overflow-hidden bg-white">
            ...
            {/* ── Blue-Purple Gradient Header ───────────────────────────── */}
            <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 px-8 py-7 text-white">
              <div className="flex items-start gap-4">
                {/* Live avatar preview */}
                <div className="relative shrink-0">
                  <div className="w-14 h-14 rounded-2xl bg-white/20 border-2 border-white/40 flex items-center justify-center text-xl font-black">
                    {hasName ? avatarInitials : <Landmark className="h-6 w-6 text-white/70" />}
                  </div>
                  {hasName && (
                    <motion.div
                      initial={{ scale: 0 }}
                ...
                <div>
                  <h1 className="text-2xl font-black tracking-tight">
                    Loan Department Registration
                  </h1>
                  <p className="text-blue-100 text-sm mt-0.5 font-medium">
                    Manage loan applications with AI-powered review and risk insights
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 flex items-center gap-3 bg-white/10 border border-white/20 rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-sm font-bold">🏦 Verified Loan Admin</p>
                  <p className="text-blue-100 text-xs font-medium mt-0.5">
                    Used only for internal loan processing and review
                  </p>
                </div>
              </div>
            </div>
            ...
            <CardContent className="p-6 sm:p-8">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Field
                      ...
                      icon={Mail}
                      required
                      hint="Auto-filled if already registered"
                    >
                      <input
                        type="email"
                        name="email"
                ...
                <section>
                  <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Landmark className="h-3.5 w-3.5" /> Loan Department Details
                  </h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Field label="Organization / Bank Name" icon={Building2} required>
                      <input
                        type="text"
                        name="orgName"
                        required
                    ...
                    <Field label="Department / Unit" icon={FileSearch} required>
                      <select
                        name="department"
                        required
                        className={`${inputCls} appearance-none cursor-pointer`}
                    ...
                    <Field label="Employee ID" icon={IdCard} required>
                      <input
                        type="text"
                        name="employeeId"
                        required
                        placeholder="LN-2024-0042"
                        className={inputCls}
                ...
                <section>
                  <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <AlertTriangle className="h-3.5 w-3.5" /> Optional Details
                  </h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Button
                  ...
                  disabled={isLoading}
                  size="lg"
                  className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold rounded-xl h-12 text-base shadow-lg shadow-blue-200 transition-all hover:-translate-y-0.5 mt-2 gap-2"
                >
                  {isLoading ? (
                    <>
                      <motion.div
