...
// ── Constants ─────────────────────────────────────────────────────────────────
const ADMIN_ROLES = ["HR Manager", "Senior Recruiter", "Talent Acquisition Lead", "HR Coordinator", "Chief People Officer"];
const DEPARTMENTS = ["Talent Acquisition", "Human Resources", "Employee Relations", "Compensation & Benefits"];
const CITIES = [
      ...
      >
        ...
        <div className="p-2">
          {[
            ...
            { icon: Settings, label: "Hiring Settings", action: () => {} },
          ...
          <div className="border-t border-slate-100 mt-1 pt-1">
            <button
              onClick={() => { onClose(); navigate("/admin/hr"); }}
...
// ── Main Component ────────────────────────────────────────────────────────────
export function JobAdminRegisterPage() {
  const navigate = useNavigate();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("jobAdminProfile", JSON.stringify(profile));
      ...
      setIsLoading(false);
      setRegistered(true);
      toast.success("HR Admin registered successfully!");
    } catch (error) {
      console.error("HR admin register error:", error);
        ...
        <div className="absolute inset-0 z-10 bg-black/60 backdrop-blur-[12px] flex items-center justify-center p-4">
          ...
          >
            <Card className="border-0 shadow-2xl rounded-2xl bg-white overflow-hidden">
              <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-blue-700 px-8 py-8 text-white text-center">
                <motion.div
              ...
              <CardContent className="p-6">
                ...
                <div className="flex gap-3">
                  <Button
                    onClick={() => navigate("/admin/hr/dashboard")}
                    className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold rounded-xl h-11 shadow-lg shadow-blue-200"
                  >
                    Go to Dashboard
                  </Button>
                  <div className="relative">
      ...
      <div className="absolute inset-0 z-20 overflow-y-auto flex items-start justify-center p-4 py-10">
        ...
        >
          <Card className="border border-white/20 shadow-[0_32px_80px_rgba(0,0,0,0.55)] rounded-2xl overflow-hidden bg-white">
            <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-blue-700 px-8 py-7 text-white">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-white/20 rounded-2xl border border-white/30">
                  <Briefcase className="h-8 w-8 text-white" />
                </div>
                <div>
                  <h1 className="text-2xl font-black tracking-tight">HR Admin Registration</h1>
                  <p className="text-blue-100 text-sm mt-0.5 font-medium">
                    Streamline hiring with AI-powered candidate evaluation
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 flex items-center gap-3 bg-white/10 border border-white/20 rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-sm font-bold">💼 Verified Hiring Admin</p>
                  <p className="text-blue-100 text-xs font-medium mt-0.5">Used for talent acquisition management</p>
                </div>
              </div>
            </div>
            ...
            <CardContent className="p-6 sm:p-8">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <section>
                  <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Building2 className="h-3.5 w-3.5" /> HR Admin Details
                  </h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Field label="Organization Name" icon={Building2} required>
                      <input
                    ...
                    <Field label="Department" icon={Briefcase} required>
                      <select
                ...
                <Button
                  ...
                  disabled={isLoading}
                  size="lg"
                  className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold rounded-xl h-12 text-base shadow-lg shadow-blue-200 transition-all hover:-translate-y-0.5 mt-2 gap-2"
                >
                  {isLoading ? (
                    <>
                      <motion.div
