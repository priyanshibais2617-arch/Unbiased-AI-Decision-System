...
export function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!localStorage.getItem("userFullName")) {
        const nameFromEmail = email.split("@")[0].replace(/[._\-\d]+/g, " ").trim();
        const capitalizedName = nameFromEmail
          .split(" ")
          .map((w: string) => w.charAt(0).toUpperCase() + w.slice(1))
          .join(" ");
        localStorage.setItem("userFullName", capitalizedName || email.split("@")[0]);
      }
  ...
  const handleGoogleLogin = () => {
    const feature = localStorage.getItem("selectedFeature");
    const redirectPath = feature ? "/complete-profile" : "/dashboard";
    const params = new URLSearchParams({ role: "user", redirect_path: redirectPath });
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <button type="button" onClick={() => setIsForgotModalOpen(true)} className="text-sm text-blue-600 hover:text-blue-700 font-medium transition-colors">
                      Forgot Password?
                    </button>
                  </div>
                  <div className="relative">
                ...
                <p className="text-center text-sm text-gray-600 pt-4">
                  ...
                  <button type="button" onClick={() => navigate('/register?mode=user')} className="font-semibold text-blue-600 hover:text-blue-700 transition-colors">
                    Register now
                  </button>
                </p>
