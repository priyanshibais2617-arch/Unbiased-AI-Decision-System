...
import {
  ArrowLeft, User, Mail, Phone, Building2, Shield, BookOpen,
  IdCard, Briefcase, MapPin, Lock, Eye, EyeOff, GraduationCap,
...
// ── Constants ─────────────────────────────────────────────────────────────────
const ADMIN_ROLES = ["Academic Coordinator", "Exam Controller", "Faculty Admin", "Department Head"];
const DEPARTMENTS = ["Computer Science", "Commerce", "Science", "Arts"];
const CITIES = [
...
function ProfileDropdown({ profile, onClose }: { profile: ProfileData; onClose: () => void }) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Header */}
        <div className="bg-gradient-to-br from-teal-600 to-emerald-700 p-5 text-white">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-teal-100 text-xs truncate">{profile.email}</p>
            </div>
          </div>
        ...
        <div className="p-2">
          {[
            ...
            { icon: Settings, label: "Academic Settings", action: () => {} },
            { icon: Bell, label: "Notifications", action: () => {} },
          ].map((item) => (
            <button
              key={item.label}
              onClick={item.action}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-teal-700 transition-colors text-left"
            >
          ...
          <div className="border-t border-slate-100 mt-1 pt-1">
            <button
              onClick={() => { onClose(); navigate("/admin/education"); }}
...
// ── Main Component ────────────────────────────────────────────────────────────
export function EducationAdminRegisterPage() {
  const navigate = useNavigate();
  ...
  const inputCls =
    "w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl bg-slate-50/70 focus:bg-white focus:ring-2 focus:ring-teal-500 focus:border-teal-400 transition-all text-sm font-medium outline-none placeholder:text-slate-300";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("eduAdminProfile", JSON.stringify(profile));
      ...
      setIsLoading(false);
      setRegistered(true);
      toast.success("Education Admin registered successfully!");
    } catch (error) {
      console.error("Education admin register error:", error);
        ...
        <div className="absolute inset-0 z-10 bg-black/60 backdrop-blur-[12px] flex items-center justify-center p-4">
          ...
          >
            <Card className="border-0 shadow-2xl rounded-2xl bg-white overflow-hidden">
              ...
              <div className="bg-gradient-to-br from-teal-500 via-emerald-500 to-green-600 px-8 py-8 text-white text-center">
                <motion.div
              ...
              <CardContent className="p-6">
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {[
                    ...
                    { label: "Institution", value: savedProfile.institutionName },
                ...
                <div className="flex gap-3">
                  <Button
                    onClick={() => navigate("/admin/education/dashboard")}
                    className="flex-1 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 text-white font-bold rounded-xl h-11 shadow-lg shadow-teal-200"
                  >
                    Go to Dashboard
                  </Button>
                  <div className="relative">
      ...
      <div className="absolute inset-0 z-20 overflow-y-auto flex items-start justify-center p-4 py-10">
        ...
        >
          <Card className="border border-white/20 shadow-[0_32px_80px_rgba(0,0,0,0.55)] rounded-2xl overflow-hidden bg-white">
            ...
            {/* ── Green Gradient Header ─────────────────────────────────── */}
            <div className="bg-gradient-to-br from-teal-500 via-emerald-500 to-green-600 px-8 py-7 text-white">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-white/20 rounded-2xl border border-white/30">
                  <GraduationCap className="h-8 w-8 text-white" />
                </div>
                <div>
                  <h1 className="text-2xl font-black tracking-tight">Education Admin Registration</h1>
                  <p className="text-teal-100 text-sm mt-0.5 font-medium">
                    Manage academic performance with AI-powered insights
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 flex items-center gap-3 bg-white/10 border border-white/20 rounded-xl px-4 py-3">
                ...
                <div>
                  <p className="text-sm font-bold">🎓 Verified Academic Admin</p>
                  <p className="text-teal-100 text-xs font-medium mt-0.5">Used for academic monitoring only</p>
                </div>
              </div>
            </div>
            ...
            <CardContent className="p-6 sm:p-8">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <section>
                  <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <BookOpen className="h-3.5 w-3.5" /> Academic Admin Details
                  </h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Field label="Institution Name" icon={Building2} required>
                      <input
                        type="text" name="institutionName" required
                        placeholder="National Institute of Technology"
                        className={inputCls}
                        value={form.institutionName} onChange={handleChange}
                      />
                    </Field>
                    ...
                    <Field label="Department" icon={BookOpen} required>
                      <select
                ...
                <Button
                  ...
                  disabled={isLoading}
                  size="lg"
                  className="w-full bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 text-white font-bold rounded-xl h-12 text-base shadow-lg shadow-teal-200 transition-all hover:-translate-y-0.5 mt-2 gap-2"
                >
                  {isLoading ? (
                    <>
                      <motion.div
