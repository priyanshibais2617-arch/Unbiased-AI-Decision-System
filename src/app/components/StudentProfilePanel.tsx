...
import { StudentLanguagePanel } from "./StudentLanguagePanel";
import { StudentAppearancePanel } from "./StudentAppearancePanel";
  ...
  return (
    <>
      ...
      <div className={`fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl z-50 flex flex-col transform transition-transform duration-300 ease-in-out`}>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 text-slate-800 custom-scrollbar pb-6">
           ...
           <div className="px-5">
              ...
              <div className="space-y-1">
                 ...
                 <button onClick={() => setActiveSettingPanel('language')} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-white hover:text-teal-600 hover:shadow-sm border border-transparent hover:border-slate-100 transition-all group">
                    <div className="flex items-center gap-3">
                       <Globe className="h-4 w-4 text-slate-400 group-hover:text-teal-500" />
                       <span className="text-sm font-semibold">Language Preference</span>
                    </div>
                    <span className="text-[10px] font-bold text-slate-400 bg-slate-200 px-2 py-0.5 rounded-md">EN</span>
                 </button>
                 <button onClick={() => setActiveSettingPanel('appearance')} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-white hover:text-teal-600 hover:shadow-sm border border-transparent hover:border-slate-100 transition-all group">
                    <div className="flex items-center gap-3">
                       <Palette className="h-4 w-4 text-slate-400 group-hover:text-teal-500" />
                       <span className="text-sm font-semibold">Appearance</span>
                    </div>
                    <span className="text-[10px] font-bold text-slate-400 bg-slate-200 px-2 py-0.5 rounded-md">LIGHT</span>
                 </button>
        ...
        {activeSettingPanel === 'language' && <StudentLanguagePanel onBack={() => setActiveSettingPanel(null)} />}
        {activeSettingPanel === 'appearance' && <StudentAppearancePanel onBack={() => setActiveSettingPanel(null)} />}
